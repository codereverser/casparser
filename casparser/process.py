...
from .regex import FOLIO_RE, HEADER_RE, SCHEME_RE, OPEN_UNITS_RE, CLOSE_UNITS_RE, TRANSACTION_RE
    ...
    for txt in lines:
        ...
        if m:
            ...
            if current_folio is None or current_folio != folio:
                if current_scheme_data and current_folio is not None:
                    folios[current_folio]['schemes'].append(current_scheme_data)
                    current_scheme_data = {}
                current_folio = folio
                folios[folio] = {'folio': current_folio, 'PAN': m.group(2).strip(),
                                 'KYC': m.group(3).strip(),
                                 'PANKYC': m.group(4).strip(), 'schemes': []}
        ...
        if m:
            ...
            if current_scheme_data.get('scheme') != scheme:
        ...
        if not current_scheme_data:
            continue
        m = re.search(OPEN_UNITS_RE, txt)
        if m:
            current_scheme_data['open'] = Decimal(m.group(1).replace(',', '_'))
            continue
        m = re.search(CLOSE_UNITS_RE, txt)
        if m:
            current_scheme_data['close'] = Decimal(m.group(1).replace(',', '_'))
            continue
        ...
        if m:
            ...
            current_scheme_data['transactions'].append((date, desc, amt, units, nav))
    if current_scheme_data:
        folios[current_folio]['schemes'].append(current_scheme_data)
    return {
